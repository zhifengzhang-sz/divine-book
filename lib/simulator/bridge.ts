...
/** Parsed book data shape (from books.yaml, generated by parser) */
interface BookData {
	school: string;
	states?: Record<string, ParserStateDef>;
	skill?: any[];
	primary_affix?: { name: string; effects: any[] };
}

/** State definition shape (from books.yaml state registry) */
interface ParserStateDef {
...
const booksPath = join(ROOT, "data/yaml/books.yaml");

/** Load parsed book data (with state registries) from books.yaml. Cached. */
function getParserBooks(): Record<string, BookData> {
	if (!_parserBooks) {
		const data = parseYaml(readFileSync(booksPath, "utf-8")) as any;
		_parserBooks = data.books ?? {};
	}
	return _parserBooks;
}
