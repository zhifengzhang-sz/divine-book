...
export function addSemantics(s: ohm.Semantics): void {
	...
	s.addOperation<Effect[]>("toEffects", {
		skillDescription(_pre, baseAttack, _sep, stateAdd, _colon, stateBody, _period, _mods, _period2) {
		...
		hpCostDot(_zsmssl, varRef, _p, _ddqqxz) {
			return [{ type: "self_hp_cost", value: varRef.extractVar, tick_interval: 1 }];
		},
		selfLostHpDamageDot(_mmdmbzc, varRef, _p, _yslqxzhqjxhqdsh) {
			return [{ type: "self_lost_hp_damage", value: varRef.extractVar, tick_interval: 1 }];
		},
		stateModifiers(_sn, _yj, _sep, _zddj, _v, _c) { return []; },
		primaryAffix(stateName, _mzc, hitsVar, _csh, _sep, _ewfj, varRef, _p, _zsyslqxzhqjxhqzdsh) {
			return [{ type: "self_lost_hp_damage", value: varRef.extractVar, every_n_hits: hitsVar.extractVar }];
		},
