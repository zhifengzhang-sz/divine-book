...
export function addSemantics(s: ohm.Semantics): void {
	...
	s.addOperation<Effect[]>("toEffects", {
		skillDescription(_pre, hpCost, _s1, baseAttack, _s2, stateAdd, _colon, stateBody) {
			return [...hpCost.toEffects(), ...baseAttack.toEffects(), { type: "state_add", state: stateAdd.extractVar }, ...stateBody.toEffects()];
		},
		...
		stateBody(counterBuff, _sep, dur) {
			return counterBuff.toEffects();
		},
		counterBuffReflect(_mmdmb, _gap, _fszssddshzd, reflectVar, _p1, _yuzishen, lostHpVar, _p2, _yssl) {
			return [{ type: "counter_buff", reflect_received_damage: reflectVar.extractVar, reflect_percent_lost_hp: lostHpVar.extractVar }];
		},
		duration(_cx, varRef, _m) { return []; },
		primaryAffix(_huifu, stateName, _zcsh, varRef, _p, _dqxz) {
			return [{ type: "lifesteal_with_parent", state: stateName.extractVar, value: varRef.extractVar }];
		},
