...
| 5 | `千锋聚灵剑`（主）+ `玄煞灵影诀`（专属）+ `惊蛰化龙`（专属） | +2% damage per 1% own HP lost + true damage per debuff stack (【怒血战意】, 【索心真诀】, $C_6$) | Self-HP exploit |
...
**→ Debuff stacks ×2 (【心魔惑言】).** Doubles debuff stacking rate from the 60% counter mechanism (【罗天魔咒】), directly feeding Slot 5's per-debuff-stack true damage (【索心真诀】). The +5.5%/5 debuff stacks (max 27.5%) is additional output. Duration +69% (#3, 【业焰】) extends 【命損】to 13.5s (covering 2 slots instead of 1), which is attractive, but debuff acceleration (【心魔惑言】) is more valuable: it enables true damage (【索心真诀】) in Slot 5.

**Alternative: Duration extension (【业焰】) for double-slot 【命損】coverage.** If 【命損】needs to cover both Slots 5 and 6, +69% duration (8s → 13.5s → $\lfloor 13.5 / 6 \rfloor = 2$ slots) achieves this. Trade-off: lose debuff stack doubling → lower true damage from 【索心真诀】.
...
| | Affix | Source | Effect |
|:---:|:------|:-------|:-------|
| **→** | 【索心真诀】 | `惊蛰化龙` exclusive | Per debuff stack: 2.1%maxHP true damage (max 10 stacks = 21%); enlightenment: +50% lost HP damage, +75% total |
...
**→ 2.1%maxHP true damage per debuff stack (【索心真诀】).** True damage ignores DR — orthogonal to all other damage modifiers. At 8 debuff stacks (from counter-debuffs 【罗天魔咒】+ stacking ×2 【心魔惑言】in Slot 4): 8 × 2.1%maxHP = 16.8%maxHP true damage. With enlightenment (+75% total → ×1.75): 29.4%maxHP true damage. +104% to debuffed targets (#2, 【引灵摘魂】) is additive with other damage bonuses, while true damage (【索心真诀】) is a separate channel entirely.

**Main position — base damage for ×2.0 self-HP multiplier (【怒血战意】) to scale:**

Both aux positions are consumed by the $C_6$ chain (【怒血战意】+ 【索心真诀】). The main needs high base damage with %maxHP — %maxHP bypasses DR and is multiplied by 【怒血战意】's +2%/1% own HP lost:
...
- True damage at 8 stacks (【索心真诀】): +29.4%maxHP (with enlightenment)
...
| 5 | `千锋聚灵剑`（主）+ `玄煞灵影诀`（专属）+ `惊蛰化龙`（专属） | +2%/1% HP lost + true damage per debuff stack (【怒血战意】, 【索心真诀】, $C_6$) | Self-HP exploit |
...
**2. DR removal (【命損】) covers the highest-output slot.** -100% DR covers exactly 1 subsequent slot. Placing it before Slot 5 — where self-HP exploit (【怒血战意】) + true damage (【索心真诀】) + `千锋聚灵剑` converge — maximizes the DR removal's value.
...
- **Gained:** +100% at 50% HP (【怒血战意】), true damage (【索心真诀】), anti-healing (【天哀灵涸】), cycle 2 buffed Burst, +142.8% ATK/DEF/HP on Slots 4–6
...
| 3 | How many debuff stacks from counter-debuffs (【罗天魔咒】) + stacking ×2 (【心魔惑言】) by Slot 5? | True damage output (【索心真诀】): 2.1%maxHP × stacks, max 21% | Medium |
...
| 2 | 【心魔惑言】 | `天轮魔经` exclusive | No extension: Slot 3 only (2s, tight) | Debuff stacks ×2 for 【索心真诀】 |
...
**→ Debuff stacks ×2 (【心魔惑言】).** Since 【业焰】takes Aux 1, 【心魔惑言】moves to Aux 2. Doubles the counter-debuff accumulation rate (60% chance per incoming attack → more 【噬心之咒】and 【断魂之咒】stacks). These feed 【索心真诀】in Slot 5 and provide the +5.5%/5 stacks damage bonus (active in cycle 2 re-cast, when this slot deals damage).
...
**→ Dispel trap (【玄心剑魄】).** Same as §1 Slot 2 Aux 1. Creates a dilemma: endure the DoT or dispel and take burst + stun. The 8s DoT (【噬心】) adds a debuff stack that persists into Slot 5, contributing to 【索心真诀】's true damage.
...
1. **True damage** (【索心真诀】) — ignores DR by definition
...
| | Affix | Source | Effect |
|:---:|:------|:-------|:-------|
| **→** | 【索心真诀】 | `惊蛰化龙` exclusive | Per debuff stack: 2.1%maxHP true damage (max 10 stacks); enlightenment: +50% HP damage, **+75% total damage** |
| 2 | 【贪狼吞星】 | Body school | +1%/1% enemy lost HP → +50% at 50% enemy lost |

**→ True damage + 75% total (【索心真诀】).** Debuff stacks at $t = 24$ are lower than §1's: counter-debuffs from 【罗天魔咒】(extended to 13.5s, $t = 6$ to $t = 19.5$) have expired. Active debuffs: 【噬心】from Slot 4 (8s from $t = 18$, 2s remaining) plus any surviving counter-debuff stacks — estimated 2–4 stacks vs §1's 6–8.

However, the enlightenment bonus provides +75% total damage regardless of stack count — a strong $C_2$-class multiplier applying to ALL damage components (ATK%, %maxHP, 【怒血战意】scaling). The +50% to self-lost-HP damage directly enhances 【怒血战意】and `千锋聚灵剑`'s built-in HP-kick. 【索心真诀】remains the best Aux 2 even with reduced debuff stacks.
...
> `千锋聚灵剑`（主）+ `玄煞灵影诀`（专属, 【怒血战意】）+ `惊蛰化龙`（专属, 【索心真诀】）
...
| 5 | `千锋聚灵剑`（主）+ `玄煞灵影诀`（专属）+ `惊蛰化龙`（专属） | +2%/1% HP lost + true damage + 75% total + 分身 bonus (【怒血战意】, 【索心真诀】) | Self-HP |
...
| 5 | `念剑诀` | `惊蛰化龙`（【索心真诀】） | `焚圣真魔咒`（【天魔真解】） | DoT |
