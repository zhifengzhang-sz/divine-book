...
# PvP vs Stronger Opponent — Weapon Support Build (Proposal 1)

**Character:** 剑九 (Sword school)
**Route:** Weapon support (Route 2)
**Method:** [weapon.support.build.md](weapon.support.build.md)
**Data:** `data/builds/剑九-pvp-vs-stronger/context.json` (generated by `bun scripts/construct-data.ts`)
...
**Data source:** `context.json → platforms[book].nativeFunctions`, `platforms[book].dBase`
...
**Data source:** `context.json → functionRankings`, `context.json → platforms[book].exclusiveAffix`
...
| Resource | Role |
|:---------|:-----|
| `data/builds/剑九-pvp-vs-stronger/context.json` | Data source for Steps 4-5 |
| `data/yaml/books.yaml` | Parsed book effects (upstream of context.json) |
| `data/raw/game.data.json` | Source of truth for all game data |
...
| [guide.build.md](guide.build.md) | Generic 6-step process + data pipeline |
