...
## Simulator trace mode

The combat simulator supports a `--trace` flag that shows the full damage derivation per hit, making it possible to verify WHY each number is what it is.

### Usage

```bash
# Full trace (all rounds)
bun app/simulate.ts --book-a 通天剑诀 --book-b 煞影千幻 --hp 5000000 --trace

# Trace a single round
bun app/simulate.ts --book-a 通天剑诀 --book-b 煞影千幻 --hp 5000000 --trace --round 1
```

### What it shows

1. **Entity snapshots** — HP, ATK, lost HP, shield at the start of each round
2. **Effect → Intent trace** — how each effect maps to intents (producers and modifiers)
3. **Per-hit damage pipeline** — every transformation step:
   - Base damage
   - Operator evaluation (per_enemy_lost_hp, per_debuff_stack, conditionals)
   - Crit bonus
   - Self damage increase
   - Damage reduction / amplification
   - Shield absorption
   - HP floor
4. **Phase breakdown** — self, opponent, counter, lifesteal, tick phases

### Programmatic access

Pass `trace: true` in `CombatConfig`. Each `RoundLog` will have a `trace?: RoundTrace` field with `HitTrace[]` arrays containing `TraceStep[]` per hit.

### Reference tests

`simulator.test.ts` includes two reference test cases with hand-verified expected values:

- **通天剑诀 vs 煞影千幻** (HP=5M) — shield absorption, per_enemy_lost_hp scaling, self_damage_taken_increase
- **千锋聚灵剑 vs 新-青元剑诀** (HP=5M) — one-round KO, pure ATK damage
