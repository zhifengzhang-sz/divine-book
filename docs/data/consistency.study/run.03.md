...
> **归一化数据提取。** 从 `data/raw/about.md` 中提取所有功法书主技能、主词缀、专属词缀以及通用词缀和修为词缀的效果数据，使用 `keyword.map.cn.md` 作为解析规范。

## 元信息

- **数据源**：`data/raw/about.md`（唯一数据源）
- **范围**：灵书效果（主技能、主词缀、专属词缀、通用词缀、修为词缀）
- **排除**：共有功能（融合伤害、悟境伤害、施法间隙）— 属于功法书基础机制，非灵书效果
...
| 修为 | 未标注数值的默认 data_state |
|:---|:---|
| 剑修 | 悟境最高加成（默认省略） |
| 法修 | 仅声明「数值受悟境影响」，无明确默认 |
| 魔修 | 悟境最高加成（默认省略）；主技能也可能受修炼阶数影响 |
| 体修 | 没有悟境（`enlightenment=0`） |

**字段格式约定**：
- 字段使用 `key=value` 格式，逗号分隔
- 数值原样复制，不做换算
- 减益（debuff）中降低属性的 `value` 使用负数
- 词缀名使用全角方头括号：【name】
- 随机效果拆分为 parent + child 行
...
| 效果类型 | 字段 | data_state |
|:---|:---|:---|
| `dot` | name=噬心, tick_interval=1, duration=8, damage_per_tick=550 | |
| `on_dispel` | name=噬心, damage=3300, stun=2 | |
...
| `shield_destroy_damage` | parent=寂灭剑心, shields_per_hit=1, percent_max_hp=12, cap_vs_monster=2400, no_shield_double_cap=4800 | |
...
| 效果类型 | 字段 | data_state |
|:---|:---|:---|
| `shield_destroy_dot` | parent=寂灭剑心, tick_interval=0.5, per_shield_damage=600, no_shield_assumed=2 | |

#### 专属词缀【追神真诀】

> 原文: 1. 本神通所添加的持续伤害触发时，额外造成目标26.5%已损失气血值的伤害
> 2. 在神通悟境（悟10境）的条件下：本神通附加目标最大气血的伤害提高50%，并且造成的伤害提升300%
...
| `self_buff` | name=仙佑, attack_bonus=70, defense_bonus=70, hp_bonus=70, duration=12 | locked |
...
| 效果类型 | 字段 | data_state |
|:---|:---|:---|
| `self_buff_extra` | buff_name=仙佑, healing_bonus=70 | locked |
...
| `conditional_debuff` | name=灵枯, condition=target_hp_below_30, target=healing_received, value=-51, duration=20 | |
...
| `dot` | name=噬心魔咒, parent=罗天魔咒, tick_interval=0.5, duration=4, percent_current_hp=7 | |
| `dot` | name=断魂之咒, parent=罗天魔咒, tick_interval=0.5, duration=4, percent_lost_hp=7 | |
...
> 原文: 魔威浩荡，引魔将之力造成五段共1500%攻击力的灵法伤害，神通施放时对敌方施加负面状态【无相魔劫】，持续12秒。【无相魔劫】期间敌方受到的神通伤害增加10%，并且【无相魔劫】时间结束时，对目标造成10%【无相魔劫】期间提升的伤害+5000%攻击力的伤害
...
| `conditional_damage` | parent=魔劫, value=105, condition=default, escalated_value=205, condition_escalated=target_has_no_healing | |
...
| 效果类型 | 字段 | data_state |
|:---|:---|:---|
| `self_hp_cost` | value=10 | enlightenment=0 |
| `base_attack` | hits=10, total=1500 | enlightenment=0 |
| `self_lost_hp_damage` | value=16, on_last_hit=true, heal_equal=true | enlightenment=0 |
| `self_buff` | name=怒灵降世, attack_bonus=20, damage_reduction=20, duration=4 | enlightenment=0 |
...
| 效果类型 | 字段 | data_state |
|:---|:---|:---|
| `self_buff_extend` | buff_name=怒灵降世, value=3.5 | enlightenment=0 |
| `periodic_cleanse` | chance=30, interval=1, cooldown=25, max_triggers=1 | enlightenment=0 |
...
| 效果类型 | 字段 | data_state |
|:---|:---|:---|
| `damage_increase` | value=380 | fusion=54 |
...
| `self_hp_cost` | value=10 | enlightenment=0 |
| `base_attack` | hits=10, total=1500 | enlightenment=0 |
| `counter_buff` | name=极怒, duration=4, reflect_received_damage=50, reflect_percent_lost_hp=15 | enlightenment=0 |
...
| 效果类型 | 字段 | data_state |
|:---|:---|:---|
| `lifesteal` | parent=极怒, value=82 | enlightenment=0 |
...
| 效果类型 | 字段 | data_state |
|:---|:---|:---|
| `all_state_duration` | value=55 | enlightenment=0 |
...
> 2. 在神通悟境的条件下：若本神通施加伤害加深类（神通伤害加深/技能伤害加深/最终伤害加深）增益状态时，则会额外对目标施加负面状态【逆转阴阳】：敌方会减少0.6倍触发属性的伤害减免类效果，持续时间与触发的增益状态相同
...
> 2. 在神通悟境的条件下：本神通每次造成伤害时，降低敌方20%最终伤害减免，持续1秒

| 效果类型 | 字段 | data_state |
|:---|:---|:---|
| `conditional_heal_buff` | condition=target_has_debuff, value=90, duration=8 | |
| `conditional_debuff` | condition=enlightenment_max, target=final_damage_reduction, value=-20, duration=1 | |
...
| 效果类型 | 字段 | data_state |
|:---|:---|:---|
| `per_self_lost_hp` | per_percent=2 | enlightenment=0 |
...
> 2. 在神通悟境的条件下：本神通附加自身已损气血的伤害提高50%，并使造成的伤害提升75%

| 效果类型 | 字段 | data_state |
|:---|:---|:---|
| `per_debuff_stack_true_damage` | per_stack=2.1, max=21 | enlightenment=0 |
...
| 效果类型 | 字段 | data_state |
|:---|:---|:---|
| `conditional_damage` | value=100, condition=target_controlled | enlightenment=0 |
...
| 效果类型 | 字段 | data_state |
|:---|:---|:---|
| `on_shield_expire` | damage_percent_of_shield=100 | enlightenment=0 |
...
| 效果类型 | 字段 | data_state |
|:---|:---|:---|
| `periodic_dispel` | interval=1, duration=10, damage_percent_of_skill=25.5, no_buff_double=true | enlightenment=0 |
...
> 【清灵】使本神通添加的增益效果强度提升20%
> 【业焰】使本神通添加的所有状态效果持续时间延长69%
> 【击瑕】本神通施放时，若敌方处于控制效果，则使本次伤害提升40%
> 【破竹】本神通施放时，每造成1段伤害，剩余段数伤害提升1%，最多提升10%
> 【金汤】本神通施放时，会在施放期间提升自身10%的伤害减免
...
> 【灵盾】使本神通添加的护盾值提升20%
> 【灵威】本神通施放后，使下一个施放的神通释放时额外获得118%的神通伤害加深
> 【摧山】本神通施放时，会使本次神通提升20%攻击力的效果
> 【通明】使本神通必定会心造成1.2倍伤害，并有25%概率将之提升至1.5倍
...
| 【福荫】 | `attack_bonus` | parent=【福荫】, value=20 | |
| 【福荫】 | `crit_damage_bonus` | parent=【福荫】, value=20 | |
| 【福荫】 | `damage_increase` | parent=【福荫】, value=20 | |
...
> 灵犀九重：使本神通必定会心造成2.97倍伤害，并有25%概率将之提升至3.97倍
...
> 明王之路：本神通施放时，会使本次神通的最终伤害加深提升50%
> 天命有归：使本神通的概率触发效果提升为必定触发，并使本神通造成的伤害提升50%
...
| 【景星天佑】 | `attack_bonus` | parent=【景星天佑】, value=55 | |
| 【景星天佑】 | `crit_damage_bonus` | parent=【景星天佑】, value=55 | |
| 【景星天佑】 | `damage_increase` | parent=【景星天佑】, value=55 | |

### 魔修

> 原文: 瑶光却邪：当本神通造成治疗效果时，会对敌方额外造成治疗量50%的伤害
> 溃魂击瑕：本神通施放时，若敌方气血值低于30%，则使本次伤害提升100%，且必定暴击
> 玄女护心：本神通造成伤害后，自身会获得1个本次神通伤害值的50%的护盾，护盾持续8秒
> 祸星无妄：本神通施放时，会对敌方添加以下任意1个减益效果：攻击降低20%、暴击率降低20%、暴击伤害降低50%
...
| 【祸星无妄】 | `attack_reduction` | parent=【祸星无妄】, value=-20 | |
| 【祸星无妄】 | `crit_rate_reduction` | parent=【祸星无妄】, value=-20 | |
| 【祸星无妄】 | `crit_damage_reduction` | parent=【祸星无妄】, value=-50 | |
...
| 词缀 | 效果类型 | 字段 | data_state |
|:---|:---|:---|:---|
| 【金刚护体】 | `self_damage_reduction_during_cast` | value=55 | enlightenment=0 |
| 【破灭天光】 | `flat_extra_damage` | value=2500 | enlightenment=0 |
| 【青云灵盾】 | `shield_strength` | value=50 | enlightenment=0 |
| 【贪狼吞星】 | `per_enemy_lost_hp` | per_percent=1 | enlightenment=0 |
| 【意坠深渊】 | `min_lost_hp_threshold` | value=11 | enlightenment=0 |
| 【意坠深渊】 | `damage_increase` | value=50 | enlightenment=0 |
...
|---------|------|---------|
| run.03 | 2026-02-25 | Fresh extraction from about.md using keyword.map.cn.md spec |
