...
> **标准化数据表。** 从 `data/raw/about.md` 提取的所有功法书主技能、主词缀、专属词缀，以及通用词缀和修为词缀的结构化数据。使用 `keyword.map.cn.md` 作为解析规范。

## 元信息

- **数据源**：`data/raw/about.md`（唯一数据源）
- **范围**：功法书主技能效果、主词缀效果、专属词缀效果、通用词缀效果、修为词缀效果
- **排除**：共有功能（融合伤害、悟境伤害、施法间隙）——这些是功法书底层机制，非灵书效果
- **计数**：9本功法书含主技能+主词缀，28本功法书含专属词缀，16个通用词缀，17个修为词缀
...
**字段格式约定**：`key=value` 对用逗号分隔。复合 data_state 用方括号：`[enlightenment=1, fusion=51]`。
...
| `on_dispel` | parent=噬心, damage=3300, stun=2 | |
...
| `conditional_damage` | parent=魔劫, value=105, condition=target_has_no_healing, escalated_value=205 | |
...
> 原文:
> 【咒书】使本神通添加的减益效果强度提升20%
> 【清灵】使本神通添加的增益效果强度提升20%
> 【业焰】使本神通添加的所有状态效果持续时间延长69% （受融合影响，数据为最高融合加成）
...
> 【灵盾】使本神通添加的护盾值提升20%
> 【灵威】本神通施放后，使下一个施放的神通释放时额外获得118%的神通伤害加深（受融合影响，数据为最高融合加成）
...
> 灵犀九重：使本神通必定会心造成2.97倍伤害，并有25%概率将之提升至3.97倍 （受融合影响，数据为最高融合加成）
...
> 原文:
> 金刚护体：本神通施放时，会在施放期间提升自身55%的伤害减免
...
|---------|------|---------|
| run.04 | 2026-02-25 | Fresh extraction from about.md using keyword.map.cn.md spec |
