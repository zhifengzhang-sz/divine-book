...
| `technique_damage_increase` | `{x}%的技能伤害加深` | `value`→%stat |
...
> **会心 ≠ 暴击。** 游戏中有三个独立的倍率机制，此前被合并为「暴击系统」。现拆分为三、三b、三c。详见三c末尾说明。
...
> **机制**：会心是作用于整个神通伤害的固定倍率。确定性触发（必定造成 `base_mult` 倍），有概率提升至 `enhanced_mult` 倍。不与暴击率、暴击伤害属性互动。示例：【灵犀九重】(×2.97)、【通明】(×1.2)。
...
| 效果类型 | 中文关键词模式 | 字段 → 单位 |
|:---|:---|:---|
| `probability_multiplier` | `{p1}%概率提升{m1}倍，{p2}%概率提升{m2}倍，{p3}%概率提升{m3}倍` | `tiers`→list of {`prob`→cumulative\_probability, `mult`→multiplier}（从高到低排列：p1 对应最高倍率） |

> **机制**：心逐倍增**所有**神通效果（伤害、治疗、减益），不仅限于伤害。它是作用于伤害链之后的外层包装。与会心为独立乘区。
>
> **累积概率说明**：`probability_multiplier`（心逐神随）的百分比为累积阈值，非独立概率。悟2境数据 x=60, y=80, z=100（合计 240% > 100%）证实此解读。含义：z% 概率至少 ×m3，y% 概率至少 ×m2，x% 概率 ×m1。边际概率：P(×m1)=x, P(×m2)=y−x, P(×m3)=z−y, P(无加成)=100−z。

---

## 三c、暴击系统（Standard Crit）
...
> **机制**：标准暴击系统——与暴击率、暴击伤害属性互动。与会心为独立乘区，两者可共存于同一灵書并独立相乘。
>
> **关于 `crit_damage_bonus`**：二中的 `crit_damage_bonus`（映射 `暴击伤害提升` / `致命伤害提升`）属于此系统。致命伤害与暴击伤害为同一属性的不同称谓。
...
6. **心逐神随的累积概率** — ~~已解决~~。悟2境数据（x=60, y=80, z=100，合计 240%）证实百分比为累积阈值。悟0境的 49%（=100−z=100−51）为无加成概率；悟2境的 0%（=100−100）表示必定至少 ×2。
...
| 2.0 | 2026-03-03 | 拆分三「暴击系统」为三（会心）、三b（心逐）、三c（暴击）。重命名 `guaranteed_crit`→`guaranteed_resonance`。二中新增 `technique_damage_increase`。 |
