...
# Divine Book Normalized Data Extraction
...
| Demon | 悟境最高加成 | "没有标识的数据为悟境最高加成" |
| Body | 没有悟境 | "没有标识的数据为没有悟境的情况" |

### Field Format Convention

- `key=value` key-value pairs, comma-separated
- `parent=X` indicates the current effect belongs to the parent effect/state named X
- Empty `data_state` column = default state for that school
- `locked` = feature/affix not yet unlocked
