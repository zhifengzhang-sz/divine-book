...
### Route: Aid Weapons (Route 2)

The 灵书 set is not an independent combat weapon — it serves as systematic support for stronger weapon systems (法宝 etc.) that carry the main damage. Combat evidence:

- **奇能诡道 > 仙露护元**: 奇能诡道 doesn't trigger from 甲元仙符's own effects, yet outperforms 48s 仙佑 — because it leverages 法宝 procs
- **Slot 3 > Slot 1 for 仙佑**: In isolation, buffing Slot 1's 4x × 3.97x crit chain is optimal. In practice, Slot 3 (t=12) wins because the weapon engine has ramped up by then
- **Weapons peak at t=12~24**: 灵书 support must be timed to this window

### Dimension Coverage

The 灵书 set must systematically provide effects across key combat dimensions to support the weapon system:
...
flowchart TB
    S1["Slot 1 (t=0)<br/>Pressure / Counter"] -->|"buy time"| D1["weapon ramp-up"]
    S2["Slot 2 (t=6)<br/>Anti-defense"] -->|"strip shields"| D2["weapons hit bare HP"]
    S3["Slot 3 (t=12)<br/>Buff + Weapon Interface"] -->|"+280% atk"| D3["weapon peak window"]
    S4["Slot 4 (t=18)<br/>Debuff Engine"] -->|"permanent amp"| D4["all sources +dmg"]
    S5["Slot 5 (t=24)<br/>Defense + Damage Amp"] -->|"魔劫 +205%"| D5["sustain + amp"]
    S6["Slot 6 (t=30)<br/>Finisher + Late Buff"] -->|"ignore reduction"| D6["insurance kill"]
```

### Per-Slot Objectives

#### Slot 1 (t=0) — Objective: **Weapon Doubling / Counter**

> **Job**: The clone (分身) is another you — it inherits your full stats, weapons, and procs. For 16s (t=0~16), your entire weapon system outputs at ×2. This is not "pressure" — it's the most direct Route 2 support possible: literally doubling weapon DPS during the ramp-up and peak windows.

**Option A: 春黎剑阵 — Weapon Doubling (current)**

| Component | Serves objective? | How |
|-----------|------------------|-----|
| 春黎剑阵 main skill | ✓ | Clone (16s, +200% dmg, 54% stats) — doubles weapon output for t=0~16 |
| 幻象剑灵 (主词缀) | ✓ | Clone takes less damage (120%), deals more (+200%) — clone survives longer, weapon doubling lasts |
| 心逐神随 (辅1 解体化形) | ✓ | 4x proc amplifies clone stats/damage — clone weapon output even higher |
| 灵犀九重 (辅2 千锋聚灵剑) | ✓ | Guaranteed crit 2.97x on opening hit |

**Weapon service**: Clone covers t=0~16, overlapping with weapon peak (t=12~24). During t=12~16, you have: clone weapon output + 仙佑 ×4 buff + weapon peak = the strongest 4-second window in the entire fight.

**Option B: 大罗幻诀 — Reactive Counter**

| Component | Serves objective? | How |
|-----------|------------------|-----|
| 大罗幻诀 main skill | ✓ | 罗天魔咒 (8s): when hit, 60% chance to apply 噬心之咒 (7% current HP/0.5s) + 断魂之咒 (7% lost HP/0.5s) back to attacker. Stacks 5 each |
| 魔魂咒界 (主词缀) | ✓ | Proc rate 30%→60%. Also applies 命損: **-100% 最终伤害减免** for 8s — weapons land at full force |
| 心逐神随 (辅1 解体化形) | ✓ | 4x all effects: DoTs become **28%/0.5s** each. 81060% atk opening hit |
| 心魔惑言 (辅2 天轮魔经) | ✓ | ×2 debuff stacks on 噬心之咒/断魂之咒/命損. Per-debuff damage +5.5%/5 stacks |

Strategy: Enemy attacks you during the exposed window → they get punished with massive DoTs and lose all damage reduction. Turns the vulnerability into an advantage. Directly serves Route 2: 命損 -100% reduction means every weapon hit lands unmitigated.

**Option A vs B trade-off:**

| | A: 春黎剑阵 | B: 大罗幻诀 |
|---|---|---|
| Against aggressive enemy | Clone takes hits for you | Enemy hurts themselves hitting you |
| Against passive enemy | Clone applies pressure anyway | 罗天魔咒 doesn't trigger if enemy doesn't attack |
| Weapon support | Clone may echo procs | 命損 -100% reduction = weapons at full power |
| Duration | Clone 16s (fixed) | 罗天魔咒 8s |
...
| 无极剑阵 (辅2 无极御剑诀) | ✓ | +555% 神通伤害 (net +205%) — amplifies all shield-stripping and DoT damage |
...
> **Job**: This is the most critical slot for Route 2. Activate the main buff timed exactly to the weapon peak window (t=12~24). Every weapon hit during this window benefits from +280% atk. 奇能诡道 directly interfaces with the weapon system — weapon procs trigger debuff duplication and enemy reduction shred.
...
| 噬心之咒 per tick | 7% current HP/0.5s | 28% current HP/0.5s | cap 5→10 stacks | **28%/0.5s × 10 stacks** |
| 断魂之咒 per tick | 7% lost HP/0.5s | 28% lost HP/0.5s | cap 5→10 stacks | **28%/0.5s × 10 stacks** |
| 命損 | -100% 最终伤害减免 | -100% (capped) | ×2 stacks | -100% reduction shred (8s) |
| 罗天魔咒 proc rate | 60% (with 魔魂咒界) | not multiplied (state property, not cast-time effect) | — | 60% per hit taken |
| 罗天魔咒 duration | 8s | — | — | 8s |

**Weapon service**: 命損 -100% 最终伤害减免 means every weapon hit lands at full power from t=0. This is the most direct Route 2 support — enemy has zero damage reduction while 罗天魔咒 is active. Window is 8s (t=0~8).
...
gantt
    ...
    噬心之咒 28%/0.5s ×10   :active, 0, 8
    断魂之咒 28%/0.5s ×10   :active, 0, 8
