...
|------|------|-------|-------|-------|
| 1，鸣鸿真魔相 | `大罗幻诀` | `解体化形`（专属） | `天轮魔经`（专属） | **new main + 辅位2** |
| 2，洪荒真剑 | `皓月剑诀` | `春黎剑阵`（专属） | `无极御剑诀`（专属） | — |
| 3，风花真法 | `甲元仙符` | `浩然星灵诀`（专属） | `周天星元`（专属） | — |
| 4，九阳真魔言 | `天魔降临咒` | `皓月剑诀`（专属） | `梵圣真魔咒`（专属） | — |
| 5，紫电真魔书/ | `天刹真魔` | `疾风九变`（【业焰】/专属） | `无相魔劫咒`（专属） | **辅位1 changed** |
| 6，造化真灵 | `十方真魄` | `惊蜇化龙`（专属） | `通天剑诀`（专属）| — |
...
> Design rationale for Slot 1 (dual-channel principle, A vs B decision) is covered in Construction Philosophy above. This section details the specific values.
...
> Matchup decision guide (complements the strategic A vs B framing in Construction Philosophy):
...
### Set 2: Debuff Count at t=30 (Slot 6)

Set 2 loses 天人五衰 and 心魔惑言 ×2 (both from 天刹真魔). The debuff pool at t=30 is thinner:

| Debuff | Applied at | Still alive at t=30? | Stacks | Notes |
|--------|-----------|---------------------|--------|-------|
| 噬心 DoT | t=6 | 悟10: ✗ (expired t=22) | 0 | — |
| 结魂锁链 | t=18 | ✓ permanent | 1 | — |
| 魔念生息 DoT | t=18 | ✓ permanent | 1 | — |
| 魔劫 | t=24 | ✓ expires t=32 | 1 | ×1 (no 心魔惑言) |
| 法宝 debuffs | various | depends on weapon | ? | — |

**Conservative estimate (灵书 only, no 法宝)**:
- 结魂锁链 (1) + 魔念生息 (1) + 魔劫 (1) = **3 stacks**
- 索心真诀: 3 × 2.1% = **6.3% enemy maxHP** as true damage
- 结魂锁链 damage amp: 3 × 0.5% = 1.5%

**With 法宝 debuffs**: Could reach 5-7 stacks → 索心真诀: **10.5-14.7% enemy maxHP** true damage. Still below Set 1A's conservative floor (12.6%).

> The loss is structural: no 天人五衰 (the biggest stack contributor in Set 1) and no ×2 doubling. Set 2 compensates through a different multiplicative zone (天威煌煌) rather than stacking.

### Set 2: Slot 6 Finisher Comparison

The core question: does Set 2's 天威煌煌 +128% 神通伤害加深 + larger 已損 pool outweigh the lost debuff scaling?

| Multiplier Zone | Set 1A (天刹真魔) | Set 1B (天刹真魔) | Set 2 (玄煞灵影诀) |
|---|---|---|---|
| **神通伤害加深** | — | — | **+128%** (天威煌煌) |
| **魔劫 damage amp** | +105/205% (8s, t=24~32) | +105/205% (12.4s, t=24~36.4) | +105/205% (8s, t=24~32) |
| **索心真诀 true dmg** | 12.6-16.8% maxHP | 8.4-16.8% maxHP | **6.3-10.5% maxHP** |
| **结魂锁链 amp** | ~4% (capped at 8 stacks) | ~3-4% | ~1.5-2.5% |
| **已損 HP pool** | base only | base only | **+~22% current HP** (6s × 4%/s compound burn) |
| **16% lost HP finisher** | 16% × base 已損 | 16% × base 已損 | **16% × (base + self-burn 已損)** |
| **神威冲云** | +36%, ignore all reduction | +36%, ignore all reduction | +36%, ignore all reduction |
| **怒灵降世** | +20% atk/reduction (7.5s) | +20% atk/reduction (7.5s) | +20% atk/reduction (7.5s) |

**Set 2 gains over Set 1A:**
- +128% 神通伤害加深 — a multiplicative zone that amplifies the 1500% atk base, the lost HP finisher, and all other Slot 6 damage
- ~22% additional current HP lost to self-burn → larger 已損 pool → bigger 16% lost HP finisher hit

**Set 2 loses vs Set 1A:**
- ~6.3-10.5% enemy maxHP true damage from 索心真诀 (vs 12.6-16.8%)
- ~2-2.5% less 结魂锁链 amp (1.5% vs 4%)
- All survivability from 不灭魔体 and 天人五衰

> The trade-off splits along damage type: Set 2 amplifies the *atk-scaling* and *已損-scaling* components of Slot 6 (which benefit from 天威煌煌's multiplicative zone), while Set 1 amplifies the *enemy-maxHP-scaling* component (索心真诀, which scales with debuff count). Against opponents with very high HP pools, Set 1's maxHP-based true damage may be more effective. Against opponents where atk-based damage still connects, Set 2's multiplicative stacking is stronger.
...
| Opponent has very high HP pool | Set 1 | 索心真诀 scales with enemy maxHP; more stacks → more true damage |
| Need late-game coverage | Set 1B | 魔劫 12.4s + 天人五衰 23.25s extend past Slot 6 |

```mermaid
%%{init: {'theme': 'base', 'themeVariables': {'primaryColor': '#3e44514D', 'primaryTextColor': '#abb2bf', 'primaryBorderColor': '#4b5263', 'lineColor': '#61afef', 'secondaryColor': '#2c313a4D', 'secondaryTextColor': '#abb2bf', 'secondaryBorderColor': '#4b5263', 'tertiaryColor': '#282c344D', 'mainBkg': '#3e44514D', 'nodeBorder': '#4b5263', 'clusterBkg': '#2c313a4D', 'clusterBorder': '#4b5263', 'titleColor': '#e5c07b', 'edgeLabelBackground': '#282c34', 'textColor': '#abb2bf', 'background': '#282c34'}}}%%
flowchart TB
    Q1{"Can opponent<br/>kill you?"}
    Q1 -->|Yes| SET1["Set 1 (天刹真魔)<br/>Survivability"]
    Q1 -->|No| SET2["Set 2 (玄煞灵影诀)<br/>Damage Race"]

    SET1 --> Q2{"Opponent plays<br/>aggressively early?"}
    Q2 -->|Yes| B["Set 1, Variation B<br/>Counter-Reflection<br/>(大罗幻诀 + 真言不灭)"]
    Q2 -->|No| Q3{"Weapons front-loaded<br/>in first 16s?"}
    Q3 -->|Yes| A["Set 1, Variation A<br/>Weapon Doubling<br/>(春黎剑阵 + 心魔惑言)"]
    Q3 -->|No| Q4{"Need reduction shred<br/>for weapons?"}
    Q4 -->|Yes| B
    Q4 -->|No| A
```
