...
function generateSnapshotTest(data: SimulationData): string {
	const { config, events, result } = data;
	const a = config.playerA;
	const b = config.playerB;
	const hpChangeCount = events.filter((e) => e.type === "HP_CHANGE").length;
...
import { loadBooksYaml, loadAffixesYaml } from "../config.js";
import type { PlayerState, StateChangeEvent } from "../types.js";

/**
 * Snapshot test: ${a.book} vs ${b.book}
 * Generated by viz snapshot generator on ${new Date().toISOString().split("T")[0]}
 * Seed: ${config.seed}
 */
test("${a.book} vs ${b.book} — deterministic snapshot", () => {
  const booksYaml = loadBooksYaml();
  const affixesYaml = loadAffixesYaml();
  const clock = new SimulationClock();
  const rng = new SeededRNG(${config.seed});

  const makePlayer = (label: string, book: string, stats: Record<string, number>) =>
    createActor(playerMachine, {
      input: {
        label,
        initialState: {
          hp: stats.hp, maxHp: stats.hp, sp: stats.sp, maxSp: stats.sp,
          spRegen: stats.spRegen, shield: 0, shields: [], destroyedShieldsTotal: 0,
          atk: stats.atk, baseAtk: stats.atk, def: stats.def, baseDef: stats.def,
          states: [], alive: true,
        } as PlayerState,
        formulas: { dr_constant: ${config.formulas.dr_constant}, sp_shield_ratio: ${config.formulas.sp_shield_ratio} },
        bookSlots: [{ slot: 1, platform: book, progression: { enlightenment: 0, fusion: 0 } }],
        booksYaml, affixesYaml, clock, rng, maxChainDepth: 10,
      },
      clock,
    });

  const playerA = makePlayer("A", "${a.book}", { hp: ${a.hp}, atk: ${a.atk}, sp: ${a.sp}, def: ${a.def}, spRegen: ${a.spRegen} });
  const playerB = makePlayer("B", "${b.book}", { hp: ${b.hp}, atk: ${b.atk}, sp: ${b.sp}, def: ${b.def}, spRegen: ${b.spRegen} });
  ...
  for (let t = 0; t <= 36000; t += 1000) {
    ...
    if (playerA.getSnapshot().status === "done" || playerB.getSnapshot().status === "done") break;
  }
  ...
  // Result assertions
  expect(aFinal.alive).toBe(${result.aFinal.alive});
  expect(bFinal.alive).toBe(${result.bFinal.alive});
  expect(aFinal.hp).toBeCloseTo(${Math.round(result.aFinal.hp)}, 0);
  expect(bFinal.hp).toBeCloseTo(${Math.round(result.bFinal.hp)}, 0);

  // HP change event count
  const hpChanges = events.filter((e) => e.type === "HP_CHANGE");
  expect(hpChanges.length).toBe(${hpChangeCount});
});
`;
}

function SnapshotButton({ data }: { data: SimulationData }) {
	const [copied, setCopied] = useState(false);

	const handleCopy = () => {
		const code = generateSnapshotTest(data);
	...
	return (
		<button type="button" onClick={handleCopy} className={btnStyle} style={{ fontSize: 11 }}>
			{copied ? "✓ Copied!" : "📋 Copy Snapshot Test"}
		</button>
	);
}

// ── Handler Coverage Panel ──────────────────────────────────────────

function CoveragePanel({ coverage }: { coverage: HandlerCoverage }) {
	const { registered, invoked } = coverage;
	const uninvoked = registered.filter((t) => !invoked.includes(t)).sort();
	const pct = registered.length > 0 ? Math.round((invoked.length / registered.length) * 100) : 0;
	const [expanded, setExpanded] = useState(false);

	return (
		<div style={{ ...tracePanel, maxHeight: expanded ? 400 : "auto", marginBottom: 16 }}>
			<div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
				<div style={sectionHeader}>
					Handler Coverage: {invoked.length}/{registered.length} ({pct}%)
				</div>
				<button type="button" onClick={() => setExpanded(!expanded)} className={btnStyle} style={{ fontSize: 10, padding: "2px 8px" }}>
					{expanded ? "▲ Collapse" : "▼ Details"}
				</button>
			</div>
			{/* Progress bar */}
			<div style={{ height: 6, background: "#2c313a", borderRadius: 3, margin: "8px 0", border: "1px solid #4b5263" }}>
				<div style={{ height: "100%", width: `${pct}%`, background: pct === 100 ? T.green : T.goldDark, borderRadius: 3, transition: "width 0.3s" }} />
			</div>
			{expanded && (
				<div style={{ display: "flex", gap: 16, fontSize: 11 }}>
					<div style={{ flex: 1 }}>
						<div style={{ color: T.green, fontWeight: "bold", marginBottom: 4 }}>Exercised ({invoked.length})</div>
						{invoked.sort().map((t) => (
							<div key={t} style={{ color: T.text, padding: "1px 0" }}>{t}</div>
						))}
					</div>
					<div style={{ flex: 1 }}>
						<div style={{ color: T.textMuted, fontWeight: "bold", marginBottom: 4 }}>Not exercised ({uninvoked.length})</div>
						{uninvoked.map((t) => (
							<div key={t} style={{ color: T.textMuted, padding: "1px 0" }}>{t}</div>
						))}
					</div>
				</div>
			)}
	...
	return (
		<>
			...
			<div style={{ display: "flex", gap: 8, marginBottom: 16, alignItems: "center" }}>
				...
				<span style={{ marginLeft: "auto" }}><SnapshotButton data={data} /></span>
			</div>
			...
			<div style={{ backgroundImage: `url('${ASSETS.panelBg}')`, backgroundSize: "500px", backgroundColor: T.bgPanel, borderRadius: 12, padding: 14, height: 400, overflowY: "auto", fontSize: 12, lineHeight: 1.6, boxShadow: `0 0 0 2px #2c3e50, 0 0 0 4px ${T.goldDark}88, 0 0 15px rgba(0,0,0,0.7), inset 0 0 30px rgba(0,0,0,0.6)` }} ref={(el) => { if (el) el.scrollTop = el.scrollHeight; }}>
				...
				<div style={{ color: T.textMuted, marginBottom: 8, whiteSpace: "pre", borderBottom: `1px solid ${T.goldDark}44`, paddingBottom: 8, textShadow: "1px 1px 2px black" }}>
					{`A: ${data.config.playerA.book}  HP=${fmt(data.config.playerA.hp)} ATK=${fmt(data.config.playerA.atk)} SP=${fmt(data.config.playerA.sp)} DEF=${fmt(data.config.playerA.def)} 悟${data.config.playerA.enlightenment}/融${data.config.playerA.fusion}\nB: ${data.config.playerB.book}  HP=${fmt(data.config.playerB.hp)} ATK=${fmt(data.config.playerB.atk)} SP=${fmt(data.config.playerB.sp)} DEF=${fmt(data.config.playerB.def)} 悟${data.config.playerB.enlightenment}/融${data.config.playerB.fusion}\nDR_K=${fmt(data.config.formulas.dr_constant)} SP→Shield=${data.config.formulas.sp_shield_ratio} seed=${data.config.seed}`}
				</div>
...
function ComparisonView({ pinned, current }: { pinned: SimulationData; current: SimulationData }) {
	const pinnedHpA = pinned.events.filter((e) => e.type === "HP_CHANGE" && e.player === "A");
	const pinnedHpB = pinned.events.filter((e) => e.type === "HP_CHANGE" && e.player === "B");
	const currentHpA = current.events.filter((e) => e.type === "HP_CHANGE" && e.player === "A");
	const currentHpB = current.events.filter((e) => e.type === "HP_CHANGE" && e.player === "B");

	const rows = Math.max(pinnedHpA.length + pinnedHpB.length, currentHpA.length + currentHpB.length);

	return (
		<div style={{ ...tracePanel, marginBottom: 16 }}>
			<div style={sectionHeader}>Side-by-Side Comparison</div>
			<div style={{ display: "flex", gap: 16, marginTop: 8 }}>
				{/* Pinned run */}
				<div style={{ flex: 1 }}>
					<div style={{ color: T.goldLight, fontWeight: "bold", fontSize: 11, marginBottom: 6, borderBottom: `1px solid ${T.goldDark}44`, paddingBottom: 4 }}>
						Pinned: {pinned.config.playerA.book} vs {pinned.config.playerB.book}
					</div>
					<div style={{ fontSize: 11 }}>
						<div style={{ color: T.textMuted, marginBottom: 4 }}>
							A: ATK={fmt(pinned.config.playerA.atk)} HP={fmt(pinned.config.playerA.hp)}
						</div>
						<div style={{ color: T.textMuted, marginBottom: 4 }}>
							B: ATK={fmt(pinned.config.playerB.atk)} HP={fmt(pinned.config.playerB.hp)}
						</div>
						<div style={{ color: pinned.result.winner ? T.green : T.textMuted, fontWeight: "bold" }}>
							{pinned.result.winner ? `Player ${pinned.result.winner} wins` : "Draw"}
						</div>
						<div style={{ color: T.textMuted, marginTop: 4 }}>
							A: HP={fmt(pinned.result.aFinal.hp)} | B: HP={fmt(pinned.result.bFinal.hp)}
						</div>
					</div>
				</div>
				{/* Current run */}
				<div style={{ flex: 1 }}>
					<div style={{ color: T.goldLight, fontWeight: "bold", fontSize: 11, marginBottom: 6, borderBottom: `1px solid ${T.goldDark}44`, paddingBottom: 4 }}>
						Current: {current.config.playerA.book} vs {current.config.playerB.book}
					</div>
					<div style={{ fontSize: 11 }}>
						<div style={{ color: T.textMuted, marginBottom: 4 }}>
							A: ATK={fmt(current.config.playerA.atk)} HP={fmt(current.config.playerA.hp)}
						</div>
						<div style={{ color: T.textMuted, marginBottom: 4 }}>
							B: ATK={fmt(current.config.playerB.atk)} HP={fmt(current.config.playerB.hp)}
						</div>
						<div style={{ color: current.result.winner ? T.green : T.textMuted, fontWeight: "bold" }}>
							{current.result.winner ? `Player ${current.result.winner} wins` : "Draw"}
						</div>
						<div style={{ color: T.textMuted, marginTop: 4 }}>
							A: HP={fmt(current.result.aFinal.hp)} | B: HP={fmt(current.result.bFinal.hp)}
						</div>
					</div>
				</div>
			</div>
			{/* Delta summary */}
			<div style={{ marginTop: 12, padding: 8, background: "rgba(0,0,0,0.3)", borderRadius: 6, fontSize: 11 }}>
				<div style={{ color: T.goldLight, fontWeight: "bold", marginBottom: 4 }}>Delta</div>
				{(() => {
					const dA = current.result.aFinal.hp - pinned.result.aFinal.hp;
					const dB = current.result.bFinal.hp - pinned.result.bFinal.hp;
					return (
						<>
							<div style={{ color: dA > 0 ? T.green : dA < 0 ? T.red : T.textMuted }}>
								A final HP: {dA > 0 ? "+" : ""}{fmt(dA)} ({dA > 0 ? "better" : dA < 0 ? "worse" : "same"})
							</div>
							<div style={{ color: dB > 0 ? T.green : dB < 0 ? T.red : T.textMuted }}>
								B final HP: {dB > 0 ? "+" : ""}{fmt(dB)} ({dB > 0 ? "better" : dB < 0 ? "worse" : "same"})
							</div>
							{pinned.result.winner !== current.result.winner && (
								<div style={{ color: T.goldLight, fontWeight: "bold", marginTop: 4 }}>
									Winner changed: {pinned.result.winner ?? "Draw"} → {current.result.winner ?? "Draw"}
								</div>
							)}
						</>
					);
...
export function App() {
	...
	const [pinnedData, setPinnedData] = useState<SimulationData | null>(null);
	...
	return (
		<div style={{ fontFamily: T.body, backgroundImage: `url('${ASSETS.fantasyBg}')`, backgroundSize: "cover", backgroundPosition: "center", backgroundAttachment: "fixed", color: T.text, minHeight: "100vh", padding: 24 }}>
			<div style={{ display: "flex", alignItems: "center", gap: 16, marginBottom: 16 }}>
				...
				{simData && !pinnedData && (
					<button type="button" className={btnStyle} onClick={() => setPinnedData(simData)}>
						📌 Pin for Comparison
					</button>
				)}
				{pinnedData && (
					<button type="button" className={btnStyle} onClick={() => setPinnedData(null)} style={{ color: T.red }}>
						✕ Clear Pin
					</button>
				)}
			</div>
			...
			{pinnedData && simData && pinnedData !== simData && (
				<ComparisonView pinned={pinnedData} current={simData} />
			)}
